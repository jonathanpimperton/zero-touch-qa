...
class SiteCrawler:
    ...
    def _rewrite_sitemap_url(self, url: str) -> str:
        """Rewrite a sitemap URL to use the scanner's target domain.

        WP Engine staging sites (e.g. trinityvethosp.wpenginepowered.com) often
        have sitemaps generated by Yoast/Rank Math that reference the production
        domain (trinityvet716.com). Rewrite those URLs so the scanner fetches
        pages from the staging site it was asked to scan.
        """
        parsed = urllib.parse.urlparse(url)
        if parsed.netloc and parsed.netloc != self.domain:
            target_scheme = urllib.parse.urlparse(self.base_url).scheme
            return urllib.parse.urlunparse((target_scheme, self.domain, parsed.path, parsed.params, parsed.query, ""))
        return url
    ...
    def _parse_sitemap_xml(self, xml_text: str, _depth: int = 0) -> list[str]:
        ...
        if child_sitemaps:
            ...
            for loc in child_sitemaps:
                ...
                if not child_url:
                    continue
                # Rewrite to target domain (staging sitemaps reference production domain)
                child_url = self._rewrite_sitemap_url(child_url)
                # Validate child sitemap is on the target domain (prevent SSRF)
                if urllib.parse.urlparse(child_url).netloc != self.domain:
                    continue
                try:
        ...
        # Flat sitemap — extract URLs directly, rewriting domain if needed
        page_urls = []
        for loc in root.findall(".//sm:url/sm:loc", ns):
            if loc.text:
                page_urls.append(self._rewrite_sitemap_url(loc.text.strip()))
        return page_urls
